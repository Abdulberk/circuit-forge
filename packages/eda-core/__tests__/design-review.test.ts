...
const diode = (id: string, p1: [string, string], p2: [string, string]): CircuitJson['components'][number] =>
    ({ id, type: 'diode', designator: id.toUpperCase(), model: 'led_red', pins: [{ pinId: p1[0], netId: p1[1] }, { pinId: p2[0], netId: p2[1] }] } as CircuitJson['components'][number]);
const cap = (id: string, a: string, b: string): CircuitJson['components'][number] =>
    ({ id, type: 'capacitor', designator: id.toUpperCase(), value: '100n', pins: [{ pinId: '1', netId: a }, { pinId: '2', netId: b }] } as CircuitJson['components'][number]);
...
describe('checkOrientationConsistency — role-consistency for diode/zener/LED only', () => {
    it('a diode declaring anode + cathode is consistent (no issue)', () => {
        const c: CircuitJson = { version: '1.0', components: [diode('d1', ['anode', 'a'], ['cathode', 'k'])], nets: nets('a', 'k') };
    ...
    it('flags a diode authored with bare 1/2 pins (orientation unspecified)', () => {
        const c: CircuitJson = { version: '1.0', components: [diode('d1', ['1', 'a'], ['2', 'k'])], nets: nets('a', 'k') };
        const r = checkOrientationConsistency(c);
