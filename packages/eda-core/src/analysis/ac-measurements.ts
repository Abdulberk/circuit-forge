 ...
 *
 * Pure: operates on the (frequency, magnitude) points the CSV parser produces for an `.ac` run
 * (y = |H(f)| = hypot(re, im), x = Hz — see csv-parser). No ngspice, no I/O. This is the metric that lets
 * "verified" mean a frequency target was actually MEASURED, not proxied by a single amplitude point (which
 * admits a ±30% error in fc).
 */
 ...
 * first-order or well-damped (Q ≤ 0.707) low- or high-pass has exactly ONE such crossing inside the swept
 * band → that is the cutoff.
 *
 * Returns `null` when there are ZERO crossings (the sweep never reaches the corner, or the response is flat)
 * or MORE than one (band-pass / a resonant peak — "the" cutoff is ambiguous). Reporting "not determinable" is
 * deliberate: certifying a guessed corner is exactly the dishonesty this metric exists to kill, so the caller
 * must treat `null` as an unmet criterion, never as a pass.
 */
export function cutoffFrequency(points: ReadonlyArray<FreqMagPoint>): number | null {
    ...
    for (let i = 0; i < pts.length - 1; i++) {
        ...
        // Interpolate the crossing in log10(frequency) (an AC dec/oct sweep is log-spaced), linear in magnitude.
        const dy = b.y - a.y;
