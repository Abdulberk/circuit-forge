...
export async function runDesignLoop(input: DesignLoopInput, deps: DesignDeps): Promise<DesignResult> {
    ...
    for (let round = 1; round <= maxRounds; round++) {
        ...
        if (simHealthy && criteria.length > 0) {
            ...
            lastAssertions = evaluateAssertions(measurements, criteria, true, circuit);
        }
 ...
 */
// The robustness tier classifier moved to eda-core (the lowest shared layer, beside the yield engine) so
// BOTH this design loop AND the /verify-design path grade a yield identically without the API depending on
// the LLM package for pure verdict math. Imported above for internal use; re-exported here so llm-core's
// existing consumers are unchanged.
export { classifyRobustness, ROBUSTNESS_PROFILES };
export type { RobustnessTier, RobustnessBars, RobustnessVerdict };
...
export async function screenCandidate(input: DesignLoopInput, deps: DesignDeps): Promise<ScreenResult> {
    ...
    try {
        ...
        if (simHealthy && criteria.length > 0) {
            ...
            attachFourierThd(measurements, result.result?.fourier);
            attachTransferFunction(measurements, result.result?.transferFunction);
            assertions = evaluateAssertions(measurements, criteria, true, circuit);
        }
    } catch {
