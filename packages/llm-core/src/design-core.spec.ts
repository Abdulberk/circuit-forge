...
describe('pollBackoffMs (sync /design-circuit sim-poll backoff — #12)', () => {
    ...
    it('issues far fewer status queries than the old fixed-1s loop over a 90s wait', () => {
        let elapsed = 0,
            polls = 0;
        for (let a = 0; elapsed < 90_000; a++) {
            elapsed += pollBackoffMs(a);
            polls++;
        }
...
describe('screenSpecsMet (S1 screen coverage gate)', () => {
    ...
    const passA = (probe: string): AssertionResult => ({
        label: '',
        probe,
        metric: 'max',
        op: 'approx',
    ...
    it('the same prompt WITH a current criterion that passes → spec-met', () => {
        expect(
            screenSpecsMet([passA('v(out)'), passA('i(R1)')], 'deliver 10mA to the load', [
                crit('v(out)'),
                crit('i(R1)'),
            ]),
        ).toBe(true);
...
describe('classifyRobustness (tolerance-aware tier layered on top of nominal verified)', () => {
    const rep = (yld: number, low: number, high: number, evaluated = 200) => ({
        yield: yld,
        evaluated,
        ci95: { low, high },
    });
...
describe('runDesignLoop — cooperative abort', () => {
    it('throws DesignAbortedError at the first checkpoint when already aborted — BEFORE any LLM or sim call', async () => {
        ...
        await expect(runDesignLoop({ prompt: 'an RC low-pass', maxRounds: 2 }, deps)).rejects.toBeInstanceOf(
            DesignAbortedError,
        );
    ...
    it('simulates the SEED directly + returns without any LLM call (would 401/network-error if generate ran)', async () => {
        const runSim = {
            ...
            getResult: jest.fn(async () => ({
                status: 'SUCCEEDED',
                result: { meta: { pointsCount: 1 }, series: [] },
...
describe('selectFinalists', () => {
    const mk = (types: string[], over: Partial<ScreenResult> = {}): ScreenResult => ({
        circuit: {
            version: '1.0',
            components: types.map((t, i) => ({ id: `c${i}`, type: t, designator: `X${i}` })),
            nets: [{ id: 'n', name: 'n' }],
        } as unknown as CircuitJson,
    ...
    it('among spec-met candidates, ranks the MORE ROBUST (wider inequality margin) first — not the thinnest (audit S2)', () => {
        ...
        const robust = mk(['resistor', 'voltage_source'], {
            specsMet: true,
            closeness: 4,
            assertions: [crit({ actual: 50, distance: 40 })],
        });
        const marginal = mk(['resistor', 'diode'], {
            specsMet: true,
            closeness: 0.05,
            assertions: [crit({ actual: 10.5, distance: 0.5 })],
        });
...
describe('preserveMetricOverlays — a fix cannot silently drop a thd/gain overlay', () => {
    ...
    const tran = (fourier?: unknown) =>
        ({ type: 'tran', stopTime: '5m', stepTime: '5u', ...(fourier ? { fourier } : {}) }) as never;
    ...
    it('does NOT carry an overlay when no thd/gain criterion is active', () => {
        const out = preserveMetricOverlays(tran(F), tran(), [
            { probe: 'out', metric: 'pp', op: 'lt', value: 1 },
        ] as never) as { fourier?: unknown };
