...
try {
    ...
    console.error(
        `🔌 Stack not reachable at ${BASE} (${e instanceof Error ? e.message : e}).\n   Bring up infra + API + a worker first (see this file's header), then re-run.`,
    );
    process.exit(2);
}
const ready = await jf('/health/ready');
must(
    'readiness: DB+Redis+S3 reachable',
    ready.d?.status === 'ok',
    `checks=${JSON.stringify(ready.d?.checks && Object.fromEntries(Object.entries(ready.d.checks).map(([k, v]) => [k, v.status])))}`,
);

// ── The core journey ──
const reg = await jf('/auth/register', {
    method: 'POST',
    headers: H(),
    body: body({ email: `e2e-dryrun-${Date.now()}@test.com`, password: 'SecurePassword123!', name: 'E2E Dry Run' }),
});
...
const proj = await jf(`/orgs/${org.d?.id}/projects`, {
    method: 'POST',
    headers: H(),
    body: body({ name: 'RC filter', description: 'e2e dry-run' }),
});
...
const simq = await jf(`/versions/${ver.d?.id}/simulations`, {
    method: 'POST',
    headers: H(),
    body: body({ analysisConfig: { type: 'tran', stepTime: '10u', stopTime: '5m' }, probes: ['in', 'out'] }),
});
must('queue simulation (explicit bare probes)', !!simq.d?.jobId, simq.d?.jobId);

const term = await poll(`/simulations/${simq.d?.jobId}`, (r) =>
    ['SUCCEEDED', 'FAILED', 'TIMED_OUT'].includes(r?.status),
);
must(
    'worker consumed the job → ngspice → terminal',
    term?.status === 'SUCCEEDED',
    `status=${term?.status} (needs a worker running)`,
);
...
const vd = await jf('/verify-design', {
    method: 'POST',
    headers: H(),
    body: body({
        circuit: divider,
        analysisConfig: { type: 'op' },
        assertions: [{ probe: 'out', metric: 'final', op: 'approx', value: 2.5, tol: 0.6 }],
        robustness: { corner: true },
    }),
});
must('verify-design verdict=pass', vd.d?.verdict === 'pass', `verdict=${vd.d?.verdict}`);
must(
    'worst-case corner ran (all 4 ±tol corners)',
    vd.d?.robustness?.worstCase?.evaluated === 4 && vd.d?.robustness?.worstCase?.passAllCorners === true,
    `evaluated=${vd.d?.robustness?.worstCase?.evaluated}, passAll=${vd.d?.robustness?.worstCase?.passAllCorners}`,
);

const vd2 = await jf('/verify-design', {
    method: 'POST',
    headers: H(),
    body: body({
        circuit: divider,
        analysisConfig: { type: 'op' },
        assertions: [{ probe: 'out', metric: 'final', op: 'gte', value: 2.45 }],
        robustness: { corner: true },
    }),
});
const wc2 = vd2.d?.robustness?.worstCase;
must(
    'worst-case finds the failing corner (informational — verdict stays pass)',
    vd2.d?.verdict === 'pass' && wc2?.passAllCorners === false && wc2?.failed >= 1,
    `verdict=${vd2.d?.verdict}, passAll=${wc2?.passAllCorners}, worst=${JSON.stringify(wc2?.worstCorners)}`,
);
