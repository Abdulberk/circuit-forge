...
for (const [name, circuit] of cases) {
    ...
    try {
        execFileSync('docker', ['run', '--rm', '-v', `${toDocker(dir)}:/work`, RUNTIME_IMAGE,
            'python3', '/usr/local/bin/fill-zones.py', '/work/board.kicad_pcb'],
    ...
    if (RENDER) {
        for (const [side, rot] of [['top', null], ['bottom', null], ['persp', '"-30,0,25"']]) {
            const out = `/work/render-${side}.png`;
            const args = ['run', '--rm', '-v', `${toDocker(dir)}:/work`, KICAD_IMAGE, 'kicad-cli', 'pcb', 'render',
                '--quality', 'high', '--width', '1400', '--height', '1050',
                ...(side === 'persp' ? ['--side', 'top', '--rotate', rot.replaceAll('"', '')] : ['--side', side]),
                '--output', out, '/work/board.kicad_pcb'];
            try {
