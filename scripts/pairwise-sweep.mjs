...
const DRIVERS = [
    {
        name: 'vsin',
        tran: { stopTime: '1m', stepTime: '1u' },
        minPP: 2,
        build: (LK) => ({
            components: [V('dv1', 'V1', 'SIN(0 2 10k)', 'dsrc', '0'), R('dr1', 'R1', '100', 'dsrc', LK)],
            nets: ['dsrc'],
        }),
    },
    {
        name: 'vpulse',
        tran: { stopTime: '1m', stepTime: '500n' },
        minPP: 3,
        build: (LK) => ({
            components: [V('dv1', 'V1', 'PULSE(0 5 0 1u 1u 40u 100u)', 'dsrc', '0'), R('dr1', 'R1', '100', 'dsrc', LK)],
            nets: ['dsrc'],
        }),
    },
    {
        ...
        build: (LK) => ({
            components: [
                V('dvp', 'VP1', 'DC 12', 'vp', '0'),
                V('dvn', 'VN1', 'DC -12', 'vn', '0'),
                V('dvs', 'VS1', 'SIN(0 2 5k)', 'dsig', '0'),
                {
                    id: 'du1',
                    type: 'subckt',
                    designator: 'U1',
    ...
    {
        name: 'gate-out',
        tran: { stopTime: '100u', stepTime: '50n' },
        minPP: 3,
        build: (LK) => ({
            components: [
                V('dvd', 'VD1', 'DC 5', 'dvdd', '0'),
                V('dva', 'VA1', 'PULSE(0 5 0 10n 10n 5u 10u)', 'da', '0'),
                V('dvb', 'VB1', 'PULSE(0 5 0 10n 10n 10u 20u)', 'db', '0'),
                {
                    id: 'dg1',
                    type: 'logic_and',
                    designator: 'XG1',
    ...
    },
    {
        ...
        build: (LK) => ({
            components: [
                V('dvd', 'VD1', 'DC 5', 'dvdd', '0'),
                V('dck', 'VC1', 'PULSE(0 5 0 10n 10n 2.5u 5u)', 'dclk', '0'),
                {
                    id: 'dff1',
                    type: 'dff',
                    designator: 'U1',
    ...
    },
    {
        ...
        build: (LK) => ({
            components: [
                V('dvc', 'VC1', 'DC 10', 'dvcc', '0'),
                R('drb1', 'RB1', '68k', 'dvcc', 'dbase'),
                R('drb2', 'RB2', '12k', 'dbase', '0'),
                R('drc', 'RC1', '1k', 'dvcc', LK),
                R('dre', 'RE1', '220', 'dem', '0'),
                {
                    id: 'dq1',
                    type: 'bjt',
                ...
                V('dvs', 'VS1', 'SIN(0 0.05 5k)', 'dsig', '0'),
                C('dci', 'CI1', '10u', 'dsig', 'dbase'),
            ],
    ...
    {
        name: 'mos-inv',
        tran: { stopTime: '500u', stepTime: '250n' },
        minPP: 3,
        build: (LK) => ({
            components: [
                V('dvd', 'VD1', 'DC 5', 'dvdd', '0'),
                V('dvg', 'VG1', 'PULSE(0 5 0 100n 100n 20u 50u)', 'dgate', '0'),
    ...
    {
        name: 'xfmr-sec',
        tran: { stopTime: '5m', stepTime: '5u' },
        minPP: 5,
        build: (LK) => ({
            components: [
                V('dvp', 'VP1', 'SIN(0 10 1k)', 'dpri0', '0'),
                R('drp', 'RP1', '10', 'dpri0', 'dpri'),
                {
                    id: 'dt1',
                    type: 'transformer',
                    designator: 'T1',
    ...
    {
        name: 'tline-far',
        tran: { stopTime: '1u', stepTime: '500p' },
        minPP: 1.5,
        build: (LK) => ({
            components: [
                V('dvs', 'VS1', 'PULSE(0 5 0 1n 1n 100n 300n)', 'dsrc', '0'),
                R('drs', 'RS1', '50', 'dsrc', 'dnear'),
                {
                    id: 'dt1',
                    type: 'tline',
                    designator: 'T1',
    ...
    },
    {
        ...
        build: (LK) => ({
            components: [
                V('dvm', 'VM1', 'SIN(0 2 2k)', 'dmod', '0'),
                R('drm', 'RM1', '10k', 'dmod', '0'),
                {
                    id: 'db1',
                    type: 'bsource',
                    designator: 'B1',
                    value: 'V=v(dmod)*v(dmod)/2',
                    pins: [
                        { pinId: '+', netId: LK },
                        { pinId: '-', netId: '0' },
                    ],
    ...
    },
    {
        ...
        build: (LK) => ({
            components: [
                V('dvs', 'VS1', 'SIN(0 1.5 3k)', 'dsig', '0'),
                R('drs', 'RS1', '10k', 'dsig', '0'),
                {
                    id: 'de1',
                    type: 'vcvs',
                    designator: 'E1',
    ...
    },
    {
        ...
        build: (LK) => ({
            components: [
                V('dvs', 'VS1', 'DC 5', 'dsup', '0'),
                V('dvc', 'VC1', 'PULSE(0 5 0 1u 1u 50u 100u)', 'dctl', '0'),
                R('drc', 'RC1', '100k', 'dctl', '0'),
                {
                    id: 'ds1',
                    type: 'switch',
                    designator: 'S1',
    ...
    {
        name: 'zener-clamp',
        tran: { stopTime: '3m', stepTime: '3u' },
        minPP: 1.2,
        build: (LK) => ({
            components: [
                V('dvs', 'VS1', 'SIN(0 10 1k)', 'dsrc', '0'),
                R('drs', 'RS1', '1k', 'dsrc', LK),
                {
                    id: 'dz1',
                    type: 'zener',
                    designator: 'Z1',
...
const LOADS = [
    ...
    { name: 'cload', build: (LK) => ({ components: [C('lc1', 'CL9', '10n', LK, '0')], nets: [], probes: [] }) },
    {
        name: 'rc-filter',
        build: (LK) => ({
            components: [R('lr1', 'RF9', '1k', LK, 'lout'), C('lc1', 'CF9', '10n', 'lout', '0')],
            nets: ['lout'],
            probes: ['v(lout)'],
        }),
    },
    {
        name: 'rl-load',
        build: (LK) => ({
            components: [R('lr1', 'RA9', '100', LK, 'lout'), L('ll1', 'LL9', '1m', 'lout', '0')],
            nets: ['lout'],
            probes: ['v(lout)'],
        }),
    },
            ...
            components: [
                {
                    ...
                    designator: 'D9',
                    pins: [
                        { pinId: 'anode', netId: LK },
                        { pinId: 'cathode', netId: 'lout' },
                    ],
                },
                R('lr1', 'RD9', '10k', 'lout', '0'),
                C('lc1', 'CD9', '100n', 'lout', '0'),
            ],
            nets: ['lout'],
            probes: ['v(lout)'],
        }),
            ...
            components: [
                {
                    ...
                    pins: [
                        { pinId: 'in1', netId: LK },
                        { pinId: 'out', netId: 'lout' },
                    ],
                },
                R('lr1', 'RG9', '10k', 'lout', '0'),
            ],
            nets: ['lout'],
            probes: ['v(lout)'],
        }),
    ...
    },
    {
        ...
        build: (LK) => ({
            components: [
                V('lvp', 'VP9', 'DC 12', 'lvp', '0'),
                V('lvn', 'VN9', 'DC -12', 'lvn', '0'),
                {
                    id: 'lu1',
                    type: 'subckt',
                    designator: 'U9',
            ...
            components: [
                {
                    ...
                    designator: 'T9',
                    properties: {
                        primaryInductance: '0.1',
                        secondaryInductance: '0.1',
                        coupling: '0.99',
                        windingResistance: '1',
                    },
                    pins: [
...
function runPair(driver, load, linkName) {
    ...
    try {
        ...
        const errs = ((r.stderr || '') + '\n' + log)
            ...
            .map((s) => s.trim())
            .filter((x) =>
                /singular matrix|no convergence|Timestep too small|Unable to find|fatal|aborted|no such/i.test(x),
            );
        let csv = '';
        try {
        ...
        if (r.status !== 0 || !csv.trim() || errs.length) {
            return {
                status: 'NGSPICE-FAIL',
                detail: `exit=${r.status} errs=${JSON.stringify(errs.slice(0, 2))} csv=${csv.trim() ? 'yes' : 'EMPTY'}`,
                netlist,
            };
        }
        ...
        if (!res.series.length || !res.meta.pointsCount)
            return { status: 'EMPTY-PARSE', detail: 'parsed zero series/rows', netlist };
...
for (const drv of DRIVERS) {
    ...
    for (const ld of LOADS) {
        ...
        if (r.status === 'PASS') {
            ...
            row.push('X');
            failures.push({ pair: `${drv.name} -> ${ld.name}`, link: 'link', ...r });
        }
    }
...
for (const dn of ['gate-out', 'dff-q'])
    for (const ln of ['gate-in', 'dff-clk'])
        hostile.push([DRIVERS.find((d) => d.name === dn), LOADS.find((l) => l.name === ln), '2x']);
