...
try {
    ...
    for (const l of ((r.stderr || '') + '\n' + log).split('\n').map((s) => s.trim()))
        if (/singular matrix|no convergence|Timestep too small|Unable to find|fatal|aborted|no such|no output/i.test(l))
            rep.errors.push(l);
    let csv = '';
    try {
    ...
    rep.series = res.series.map((s) => {
        ...
        return {
            ...
            max: round(max),
            pp: round(max - min),
            final: round(ys[ys.length - 1]),
        };
    });
    if (analysis.type === 'tran') {
        const ps = parseSpiceValue(analysis.stopTime);
        const want = ps.isValid ? ps.value : 0;
        const lastT = Math.max(0, ...res.series.map((s) => (s.points.length ? s.points[s.points.length - 1].x : 0)));
        if (want > 0 && lastT > 0 && lastT < 0.9 * want) {
            rep.truncated = true;
            rep.errors.push(`truncated at t=${lastT.toExponential(2)} of ${analysis.stopTime}`);
        }
    }
