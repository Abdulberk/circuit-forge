...
add('E-physics', '.noise with NO AC magnitude on the source → graceful (no crash; finite-or-null totals)', () => ({ circuit: circuit([V('v1', 'V1', 'DC 0', 'in', '0'), R('r1', 'R1', '1k', 'in', 'out'), C('c1', 'C1', '159n', 'out', '0')], N('in', 'out')), analysis: { type: 'noise', output: 'v(out)', inputSource: 'V1', variation: 'dec', points: 10, startFreq: '1', stopFreq: '100k' }, probes: ['v(out)'] }), (r) => { const o = r.noise?.onoiseTotalV; return r.noise && (o === null || Number.isFinite(o)) ? ok() : fail(`noise-no-AC not graceful: ${JSON.stringify(r.noise)} exit=${r.exit}`); });
add('E-physics', '.meas WHEN threshold never reached → that measure marked failed, run still succeeds', () => ({ circuit: circuit([V('v1', 'V1', 'SIN(0 1 1k)', 'in', '0'), R('r1', 'R1', '1k', 'in', '0')], N('in')), analysis: { type: 'tran', stopTime: '5m', stepTime: '5u', measurements: [{ name: 'never', type: 'when', probe: 'v(in)', value: 999, edge: 'rise' }] }, probes: ['v(in)'] }), (r) => { const m = (r.measurements || []).find((x) => x.name === 'never'); return baseOk(r).pass && m && m.failed === true && m.value === null ? ok() : fail(`meas-WHEN-fail: runOk=${baseOk(r).pass}, measure=${JSON.stringify(m)}`); });
add('E-physics', '.tf with a non-existent source → run still succeeds, transferFunction simply absent (no crash)', () => ({ circuit: circuit([V('v1', 'V1', 'DC 5', 'in', '0'), R('r1', 'R1', '1k', 'in', 'out'), R('r2', 'R2', '2k', 'out', '0')], N('in', 'out')), analysis: { type: 'op', tf: { output: 'v(out)', inputSource: 'V99' } }, probes: ['v(out)'] }), (r) => baseOk(r).pass && !r.transferFunction ? ok() : fail(`tf-bad-source: runOk=${baseOk(r).pass}, tf=${JSON.stringify(r.transferFunction)}`));
// QA-hardening: ACTIVE-circuit breadth — .tf on a non-inverting op-amp should give the closed-loop gain 1+Rf/Rg.
add('E-physics', '.tf on a non-inverting op-amp: gain = 1+Rf/Rg = 2 (active circuit, real ngspice)', () => ({ circuit: circuit([V('vin', 'V1', 'DC 0.1', 'in', '0'), V('vcc', 'VCC', 'DC 15', 'vcc', '0'), V('vee', 'VEE', 'DC -15', 'vee', '0'), R('rf', 'RF', '10k', 'out', 'fb'), R('rg', 'RG', '10k', 'fb', '0'), { id: 'u1', type: 'subckt', designator: 'U1', model: 'OPAMPGEN', pins: [{ pinId: 'out', netId: 'out' }, { pinId: 'in+', netId: 'in' }, { pinId: 'in-', netId: 'fb' }, { pinId: 'vcc', netId: 'vcc' }, { pinId: 'vee', netId: 'vee' }] }], N('in', 'vcc', 'vee', 'out', 'fb')), analysis: { type: 'op', tf: { output: 'v(out)', inputSource: 'V1' } }, probes: ['v(out)'] }), (r) => near(r.transferFunction?.gain, 2, 0.3) ? ok() : fail(`tf-opamp gain=${r.transferFunction?.gain} want ~2 = 1+Rf/Rg; tf=${JSON.stringify(r.transferFunction)}`));
