...
const { runMonteCarloBatch } = await import(
    new URL('../apps/worker-sim/dist/simulation/montecarlo-runner.js', import.meta.url)
);
const { runSweepBatch } = await import(new URL('../apps/worker-sim/dist/simulation/sweep-runner.js', import.meta.url));
const { runCornerBatch } = await import(
    new URL('../apps/worker-sim/dist/simulation/corner-runner.js', import.meta.url)
);

const gnd = { id: 'gnd', name: 'GND', isGround: true };
const CJ = (comps, nets) => ({ version: '1.0', components: comps, nets });
const V = (id, des, val, a, b) => ({
    id,
    type: 'voltage_source',
    designator: des,
    value: val,
...
const rc = CJ(
    [V('v1', 'V1', 'SIN(0 1 1k)', 'in', 'gnd'), R('r1', 'R1', '1k', 'in', 'out'), Cap('c1', 'C1', '1u', 'out', 'gnd')],
    [{ id: 'in' }, { id: 'out' }, gnd],
);
const divider = CJ(
    [V('v1', 'V1', 'DC 5', 'in', 'gnd'), R('r1', 'R1', '1k', 'in', 'out'), R('r2', 'R2', '2k', 'out', 'gnd')],
    [{ id: 'in' }, { id: 'out' }, gnd],
);
const noiseRc = CJ(
    [V('v1', 'V1', 'DC 0 AC 1', 'in', 'gnd'), R('r1', 'R1', '1k', 'in', 'out'), Cap('c1', 'C1', '159n', 'out', 'gnd')],
    [{ id: 'in' }, { id: 'out' }, gnd],
);
...
const cases = [
    {
        ...
        type: 'tran',
        ok: (r) => (r.result?.fourier?.[0]?.thd ?? -1) >= 0,
    },
    {
        name: 'meas (tran)',
        circuit: sineR,
        analysis: {
            ...
            stopTime: '5m',
            stepTime: '5u',
            measurements: [{ name: 'vpk', type: 'max', probe: 'v(in)' }],
        },
        type: 'tran',
        ok: (r) => Number.isFinite(r.result?.measurements?.find((m) => m.name === 'vpk')?.value),
    },
    {
        name: 'tf (op)',
        circuit: divider,
        ...
        type: 'op',
        ok: (r) => Math.abs((r.result?.transferFunction?.gain ?? 0) - 2 / 3) < 0.02,
    },
    {
        name: 'noise',
        circuit: noiseRc,
        ...
        type: 'noise',
        ok: (r) =>
            r.result?.series?.some((s) => s.name === 'onoise_spectrum') &&
            Number.isFinite(r.result?.noise?.onoiseTotalV),
    },
    {
        name: 'sens',
        circuit: divider,
        analysis: { type: 'sens', output: 'v(out)' },
        type: 'sens',
        ok: (r) => (r.result?.sensitivity?.entries?.length ?? 0) > 0,
    },
];
...
for (const tc of cases) {
    ...
    try {
        r = await runSimulation({
            jobId: `qa-${tc.name.replace(/\W+/g, '-')}-${process.pid}`,
            netlist,
            probeNames: [],
            analysisType: tc.type,
        });
    } catch (e) {
    ...
    if (!pass) fail++;
    const detail = tc.name.startsWith('fourier')
        ? `thd=${r.result?.fourier?.[0]?.thd}`
        : tc.name.startsWith('tf')
          ? `gain=${r.result?.transferFunction?.gain}`
          : tc.name.startsWith('noise')
            ? `onoiseTotal=${r.result?.noise?.onoiseTotalV}, series=${r.result?.series?.map((s) => s.name).join('/')}`
            : tc.name.startsWith('sens')
              ? `entries=${r.result?.sensitivity?.entries?.length}`
              : `vpk=${r.result?.measurements?.find((m) => m.name === 'vpk')?.value}`;
    console.log(
        `${pass ? '✅' : '❌'}  ${tc.name}: success=${r.success}, ${detail}${r.error ? ', err=' + r.error : ''}`,
    );
}
...
const square = CJ(
    [V('v1', 'V1', 'PULSE(-1 1 0 1n 1n 0.5m 1m)', 'out', 'gnd'), R('r1', 'R1', '1k', 'out', 'gnd')],
    [{ id: 'out' }, gnd],
);
...
async function nominalGate(name, circuit, ana, crit, wantPass) {
    ...
    const ex = resolveGenericModels(c);
    if (ex.length) c.models = [...(c.models ?? []), ...ex];
    const r = await runSimulation({
        jobId: `thdgate-${name.replace(/\W+/g, '-')}-${process.pid}`,
        netlist: generateNetlist(c, ana),
        probeNames: [],
        analysisType: 'tran',
    });
    ...
    if (!ok) fail++;
    console.log(
        `${ok ? '✅' : '❌'}  nominal THD-gate [${name}]: pass=${res?.pass} (want ${wantPass}), actual=${res?.actual}%`,
    );
}
...
const tolRc = CJ(
    [
        V('v1', 'V1', 'SIN(0 1 1k)', 'in', 'gnd'),
        R('r1', 'R1', '1k', 'in', 'out', 0.05),
        Cap('c1', 'C1', '1u', 'out', 'gnd', 0.05),
    ],
    [{ id: 'in' }, { id: 'out' }, gnd],
);
async function mcThd(name, crit, wantHighYield) {
    const c = JSON.parse(JSON.stringify(tolRc));
    const ex = resolveGenericModels(c);
    if (ex.length) c.models = [...(c.models ?? []), ...ex];
    const mc = await runMonteCarloBatch({
        jobId: `mcthd-${name.replace(/\W+/g, '-')}-${process.pid}`,
        circuit: c,
        analysis: fourTran('5m', '5u'),
        criteria: [crit],
        n: 15,
        seed: 1,
    });
    ...
    if (!ok) fail++;
    console.log(
        `${ok ? '✅' : '❌'}  robust-THD MC [${name}]: yield=${mc.yield}, evaluated=${mc.evaluated} (want ${wantHighYield ? 'high' : 'low'})`,
    );
}
...
const opamp = (tol) =>
    CJ(
        [
            V('vin', 'V1', 'DC 0.1', 'in', 'gnd'),
            V('vcc', 'VCC', 'DC 15', 'vcc', 'gnd'),
            V('vee', 'VEE', 'DC -15', 'vee', 'gnd'),
            R('rf', 'RF', '10k', 'out', 'fb', tol),
            R('rg', 'RG', '10k', 'fb', 'gnd', tol),
            {
                id: 'u1',
                type: 'subckt',
                designator: 'U1',
...
async function gainGate(name, crit, wantPass) {
    const c = JSON.parse(JSON.stringify(opamp()));
    const ex = resolveGenericModels(c);
    if (ex.length) c.models = [...(c.models ?? []), ...ex];
    const r = await runSimulation({
        jobId: `gaingate-${name.replace(/\W+/g, '-')}-${process.pid}`,
        netlist: generateNetlist(c, opAna),
        probeNames: [],
        analysisType: 'op',
    });
    ...
    if (!ok) fail++;
    console.log(
        `${ok ? '✅' : '❌'}  nominal gain-gate [${name}]: pass=${res?.pass} (want ${wantPass}), actual=${res?.actual}`,
    );
}
...
async function mcGain(name, crit, wantHigh) {
    const c = JSON.parse(JSON.stringify(opamp(0.05)));
    const ex = resolveGenericModels(c);
    if (ex.length) c.models = [...(c.models ?? []), ...ex];
    const mc = await runMonteCarloBatch({
        jobId: `mcgain-${name.replace(/\W+/g, '-')}-${process.pid}`,
        circuit: c,
        analysis: opAna,
        criteria: [crit],
        n: 15,
    ...
    if (!ok) fail++;
    console.log(
        `${ok ? '✅' : '❌'}  robust-gain MC [${name}]: yield=${mc.yield}, evaluated=${mc.evaluated} (want ${wantHigh ? 'high' : 'low'})`,
    );
}
...
// divider out = 5·R2/(R1+R2) = 5·2k/(R1+2k). Sweep R1 1k..9k (1k,3k,5k,7k,9k); out≥2V ⇔ R1≤3k → pass at 1k,3k.
{
    ...
    const sw = await runSweepBatch({
        jobId: `sweep-${process.pid}`,
        circuit: divider,
        analysis: { type: 'op' },
        criteria: [crit],
        sweep,
    ...
    const ok =
        ...
        sw.passed === 2 &&
        sw.passAll === false &&
        sw.passRange?.lo === 1000 &&
        sw.passRange?.hi === 3000;
    if (!ok) fail++;
    console.log(
        `${ok ? '✅' : '❌'}  parametric sweep [R1 1k..9k, out≥2V]: passed=${sw.passed}/${sw.evaluated}, passRange=${sw.passRange ? `[${sw.passRange.lo},${sw.passRange.hi}]` : 'none'}, passAll=${sw.passAll}, errored=${sw.errored}`,
    );
}
...
// divider out = 5·R2/(R1+R2), both ±10%. out ≥ 2.45V passes at nominal (2.5) but breaks at R1-hi/R2-lo (2.25).
{
    const cornerDiv = CJ(
        [
            V('v1', 'V1', 'DC 5', 'in', 'gnd'),
            R('r1', 'R1', '1k', 'in', 'out', 0.1),
            R('r2', 'R2', '1k', 'out', 'gnd', 0.1),
        ],
        [{ id: 'in' }, { id: 'out' }, gnd],
    );
    const crit = { probe: 'v(out)', metric: 'max', op: 'gte', value: 2.45 };
    const wc = await runCornerBatch({
        jobId: `corner-${process.pid}`,
        circuit: cornerDiv,
        analysis: { type: 'op' },
        criteria: [crit],
        corner: {},
    ...
    if (!ok) fail++;
    console.log(
        `${ok ? '✅' : '❌'}  worst-case corner [R1,R2 ±10%, out≥2.45V]: passed=${wc.passed}/${wc.evaluated}, passAllCorners=${wc.passAllCorners}, worst={R1:hi,R2:lo} found=${worst}, errored=${wc.errored}`,
    );
}
