...
add('bjt', 'npn CE midband AC gain |Av|>20 (catches C<->E swap)', () => ({ circuit: circuit([V('v1', 'V1', 'DC 10', 'vcc', '0'), R('rb1', 'RB1', '68k', 'vcc', 'b'), R('rb2', 'RB2', '12k', 'b', '0'), R('rc', 'RC', '1k', 'vcc', 'col'), R('re', 'RE', '220', 'em', '0'), C('ce', 'CE', '100u', 'em', '0'), { id: 'vin', type: 'voltage_source', designator: 'VIN', value: 'AC 1', pins: [{ pinId: '+', netId: 'sig' }, { pinId: '-', netId: '0' }] }, C('cin', 'CIN', '10u', 'sig', 'b'), { id: 'q1', type: 'bjt', designator: 'Q1', model: 'QGENNPN', pins: [{ pinId: 'c', netId: 'col' }, { pinId: 'b', netId: 'b' }, { pinId: 'e', netId: 'em' }] }], N('vcc', 'sig', 'b', 'col', 'em')), analysis: AC, probes: ['v(col)'] }), (r) => baseOk(r).pass && r.series[0].max > 20 ? ok() : fail(`CE midband |Av|=${r.series[0]?.max?.toFixed(1)} want >20 (gm*Rc; a C<->E swap collapses it to ~3)`));
add('bjt', 'npn emitter-follower AC gain ~1 (<unity)', () => ({ circuit: circuit([V('v1', 'V1', 'DC 10', 'vcc', '0'), R('rb1', 'RB1', '47k', 'vcc', 'b'), R('rb2', 'RB2', '47k', 'b', '0'), R('re', 'RE', '1k', 'em', '0'), { id: 'vin', type: 'voltage_source', designator: 'VIN', value: 'AC 1', pins: [{ pinId: '+', netId: 'sig' }, { pinId: '-', netId: '0' }] }, C('cin', 'CIN', '10u', 'sig', 'b'), { id: 'q1', type: 'bjt', designator: 'Q1', model: 'QGENNPN', pins: [{ pinId: 'c', netId: 'vcc' }, { pinId: 'b', netId: 'b' }, { pinId: 'e', netId: 'em' }] }], N('vcc', 'sig', 'b', 'em')), analysis: AC, probes: ['v(em)'] }), (r) => baseOk(r).pass && r.series[0].max > 0.8 && r.series[0].max < 1.05 ? ok() : fail(`follower |Av|=${r.series[0]?.max?.toFixed(3)} want ~1 (0.8-1.05)`));

add('bjt', 'npn current mirror Iout/Iref ~1 (catches mirror-transistor C<->E swap)', () => ({ circuit: circuit([V('v1', 'V1', 'DC 10', 'vcc', '0'), R('rref', 'RREF', '10k', 'vcc', 'ref'), R('rload', 'RLOAD', '1k', 'vcc', 'out'), { id: 'q1', type: 'bjt', designator: 'Q1', model: 'QGENNPN', pins: [{ pinId: 'c', netId: 'ref' }, { pinId: 'b', netId: 'ref' }, { pinId: 'e', netId: '0' }] }, { id: 'q2', type: 'bjt', designator: 'Q2', model: 'QGENNPN', pins: [{ pinId: 'c', netId: 'out' }, { pinId: 'b', netId: 'ref' }, { pinId: 'e', netId: '0' }] }], N('vcc', 'ref', 'out')), analysis: OP, probes: ['i(RREF)', 'i(RLOAD)'] }), (r) => { const ratio = Math.abs(r.series[1]?.final) / Math.abs(r.series[0]?.final); return baseOk(r).pass && ratio > 0.85 && ratio < 1.2 ? ok() : fail(`Iout/Iref=${ratio?.toFixed(3)} want ~1 (a mirror-transistor C<->E swap → ~0.45)`); });
add('bjt', 'pnp CE midband AC gain |Av|>20 (catches C<->E swap)', () => ({ circuit: circuit([V('v1', 'V1', 'DC 10', 'vcc', '0'), R('rb1', 'RB1', '12k', 'vcc', 'b'), R('rb2', 'RB2', '68k', 'b', '0'), R('rc', 'RC', '1k', 'col', '0'), R('re', 'RE', '220', 'vcc', 'em'), C('ce', 'CE', '100u', 'vcc', 'em'), { id: 'vin', type: 'voltage_source', designator: 'VIN', value: 'AC 1', pins: [{ pinId: '+', netId: 'sig' }, { pinId: '-', netId: '0' }] }, C('cin', 'CIN', '10u', 'sig', 'b'), { id: 'q1', type: 'bjt', designator: 'Q1', model: 'QGENPNP', pins: [{ pinId: 'c', netId: 'col' }, { pinId: 'b', netId: 'b' }, { pinId: 'e', netId: 'em' }] }], N('vcc', 'sig', 'b', 'col', 'em')), analysis: AC, probes: ['v(col)'] }), (r) => baseOk(r).pass && r.series[0].max > 20 ? ok() : fail(`PNP CE |Av|=${r.series[0]?.max?.toFixed(1)} want >20 (a C<->E swap collapses it to ~3)`));
...
add('igbt', 'IGBT saturation current ~15A @ Vge=10 (guards KP, audit #9)', () => ({ circuit: circuit([V('vce', 'VCE', 'DC 8', 'c', '0'), V('vge', 'VGE', 'DC 10', 'g', '0'), { id: 'x1', type: 'subckt', designator: 'X1', model: 'IGBTGEN', pins: [{ pinId: 'c', netId: 'c' }, { pinId: 'g', netId: 'g' }, { pinId: 'e', netId: '0' }] }], N('c', 'g')), analysis: OP, probes: ['i(VCE)'] }), (r) => baseOk(r).pass && near(Math.abs(r.series[0].final), 15, 3) ? ok() : fail(`Ice=${Math.abs(r.series[0]?.final).toFixed(1)}A want ~15 (KP=0.99; a regression to KP=5 -> ~75A)`));
add('scr', 'SCR latches then HOLDS after the gate is removed (catches anode<->cathode swap)', () => ({ circuit: circuit([V('vaa', 'VAA', 'DC 10', 'vcc', '0'), R('rl', 'RL', '1k', 'vcc', 'a'), V('vg', 'VG', 'PULSE(0 3 1m 1u 1u 0.5m 20m)', 'g', '0'), { id: 'x1', type: 'subckt', designator: 'X1', model: 'SCRGEN', pins: [{ pinId: 'anode', netId: 'a' }, { pinId: 'gate', netId: 'g' }, { pinId: 'cathode', netId: '0' }] }], N('vcc', 'a', 'g')), analysis: { type: 'tran', stopTime: '5m', stepTime: '5u' }, probes: ['v(a)'] }), (r) => baseOk(r).pass && r.series[0].max > 8 && r.series[0].final < 3 ? ok() : fail(`SCR v(a) max=${r.series[0]?.max?.toFixed(1)} final=${r.series[0]?.final?.toFixed(1)}: want blocked-then-latched (max~10, final<3); an anode<->cathode swap → final~10`));
...
add('subckt', 'opamp open-loop sign (in+/in- polarity)', () => ({ circuit: circuit([V('vcc', 'VCC', 'DC 15', 'vcc', '0'), V('vee', 'VEE', 'DC -15', 'vee', '0'), V('vp', 'VP', 'DC 1', 'p', '0'), V('vn', 'VN', 'DC 2', 'n', '0'), { id: 'u1', type: 'subckt', designator: 'U1', model: 'OPAMPGEN', pins: [{ pinId: 'out', netId: 'out' }, { pinId: 'in+', netId: 'p' }, { pinId: 'in-', netId: 'n' }, { pinId: 'vcc', netId: 'vcc' }, { pinId: 'vee', netId: 'vee' }] }], N('vcc', 'vee', 'p', 'n', 'out')), analysis: OP, probes: ['v(out)'] }), (r) => baseOk(r).pass && r.series[0].final < -5 ? ok() : fail(`open-loop V+(1)<V-(2) -> v(out)=${r.series[0]?.final?.toFixed(1)} want negative rail (a +/- port swap flips it to +15)`));
...
add('transformer', 'transformer turns ratio Vsec/Vpri ~0.5 (catches primary<->secondary swap)', () => ({ circuit: circuit([V('v1', 'V1', 'SIN(0 10 1k)', 'pri', '0'), { id: 't1', type: 'transformer', designator: 'T1', properties: { primaryInductance: '1', secondaryInductance: '0.25', coupling: '0.999' }, pins: [{ pinId: 'p+', netId: 'pri' }, { pinId: 'p-', netId: '0' }, { pinId: 's+', netId: 'sec' }, { pinId: 's-', netId: '0' }] }, R('rl', 'RL', '100k', 'sec', '0')], N('pri', 'sec')), analysis: { type: 'tran', stopTime: '3m', stepTime: '2u' }, probes: ['v(pri)', 'v(sec)'] }), (r) => { const ratio = r.series[1]?.pp / r.series[0]?.pp; return baseOk(r).pass && ratio > 0.4 && ratio < 0.6 ? ok() : fail(`Vsec/Vpri=${ratio?.toFixed(3)} want ~0.5=sqrt(Ls/Lp) (a p<->s winding swap → ~2.0)`); });
