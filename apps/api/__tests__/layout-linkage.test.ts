...
describe('Layout ↔ version linkage (GET /layouts, POST /layouts versionId)', () => {
    ...
    let tokenB = '',
        ...
        orgB = '';
    let orgA2 = '',
        projA2 = ''; // a SECOND org for user A → proves org is derived from the version, not first-org
    let versionedJobId = '',
        adhocJobId = '';
    ...
    beforeAll(async () => {
        ...
        const regA = await request(server)
            .post('/auth/register')
            .send({ email: `lay-a-${Date.now()}@test.com`, password: 'SecurePassword123!', name: 'Lay A' })
            .expect(201);
        ...
        const regB = await request(server)
            .post('/auth/register')
            .send({ email: `lay-b-${Date.now()}@test.com`, password: 'SecurePassword123!', name: 'Lay B' })
            .expect(201);
    ...
    it('POST /layouts with a versionId → 202 QUEUED; the server derives project + org from the version', async () => {
        const r = await request(server)
            .post('/layouts')
            .set('Authorization', `Bearer ${tokenA}`)
            .send({ circuit, versionId: versionA })
            .expect(202);
    ...
    it('THE RELOAD: GET /layouts?versionId= re-finds the layout from the durable versionId alone (no jobId in hand)', async () => {
        const r = await request(server)
            .get(`/layouts?versionId=${versionA}`)
            .set('Authorization', `Bearer ${tokenA}`)
            .expect(200);
    ...
    it('the projectId filter returns the versioned job', async () => {
        const r = await request(server)
            .get(`/layouts?projectId=${projectA}`)
            .set('Authorization', `Bearer ${tokenA}`)
            .expect(200);
    ...
    it("the layout org is DERIVED from the version, not the actor's first org (multi-org)", async () => {
        ...
        orgA2 = (
            await request(server)
                .post('/orgs')
                .set('Authorization', `Bearer ${tokenA}`)
                .send({ name: 'Lay Org A2' })
                .expect(201)
        ).body.id;
        projA2 = (
            await request(server)
                .post(`/orgs/${orgA2}/projects`)
                .set('Authorization', `Bearer ${tokenA}`)
                .send({ name: 'Lay Proj A2' })
                .expect(201)
        ).body.id;
        const verA2 = (
            await request(server)
                .post(`/projects/${projA2}/versions`)
                .set('Authorization', `Bearer ${tokenA}`)
                .send({ circuitJson: circuit, uiJson: {} })
                .expect(201)
        ).body.id;
        ...
        const j = await request(server)
            .post('/layouts')
            .set('Authorization', `Bearer ${tokenA}`)
            .send({ circuit, versionId: verA2 })
            .expect(202);
        const row = await prisma.layoutJob.findUnique({
            where: { id: j.body.jobId },
            select: { orgId: true, versionId: true, projectId: true },
        });
    ...
    it('POST /layouts with an unknown versionId → 404 (no silent untagged job)', async () => {
        await request(server)
            .post('/layouts')
            .set('Authorization', `Bearer ${tokenA}`)
            .send({ circuit, versionId: '00000000-0000-0000-0000-000000000000' })
            .expect(404);
    });
