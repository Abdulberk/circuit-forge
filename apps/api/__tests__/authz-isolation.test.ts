...
describe('Authz / multi-tenant isolation (IDOR)', () => {
    ...
    beforeAll(async () => {
        ...
        versionA = (
            await post(`/projects/${projectA}/versions`, tokenA, { circuitJson: circuit, uiJson: {} }).expect(201)
        ).body.id;
        simJobA = (
            await post(`/versions/${versionA}/simulations`, tokenA, {
                analysisConfig: { type: 'op' },
                probes: ['out'],
            }).expect(201)
        ).body.jobId;
        designJobA = (await post('/design-jobs', tokenA, { prompt: 'a 5V divider', maxRounds: 1 }).expect(202)).body
            .jobId;
    ...
    describe("cross-org access is DENIED (B cannot touch A's resources)", () => {
        ...
        it('GET + PATCH + DELETE /projects/:projectIdA', async () => {
            ...
            denied(
                (await request(server).delete(`/projects/${projectA}`).set('Authorization', `Bearer ${tokenB}`)).status,
            );
        });
        ...
        it('POST /versions/:versionIdA/simulations', async () => {
            denied(
                (await post(`/versions/${versionA}/simulations`, tokenB, { analysisConfig: { type: 'op' } })).status,
            );
        });
        ...
        it('GET + DELETE /design-jobs/:designJobIdA', async () => {
            ...
            denied(
                (await request(server).delete(`/design-jobs/${designJobA}`).set('Authorization', `Bearer ${tokenB}`))
                    .status,
            );
        });
