...
describe('resolveEmailTransport (dynamic provider selection)', () => {
    ...
    it('honors an explicit EMAIL_PROVIDER=log', () => {
        expect(resolveEmailTransport(cfg({ EMAIL_PROVIDER: 'log', EMAIL_FROM: 'a@b.com', SES_REGION: 'eu-west-1' })).name).toBe('log');
    });

    it('selects SES when EMAIL_PROVIDER=ses with EMAIL_FROM + a region', () => {
        expect(resolveEmailTransport(cfg({ EMAIL_PROVIDER: 'ses', EMAIL_FROM: 'no-reply@cf.io', SES_REGION: 'eu-west-1' })).name).toBe('ses');
        // AWS_REGION is an accepted region source too
        expect(resolveEmailTransport(cfg({ EMAIL_PROVIDER: 'ses', EMAIL_FROM: 'no-reply@cf.io', AWS_REGION: 'us-east-1' })).name).toBe('ses');
    });
    ...
    it('selects SMTP when EMAIL_PROVIDER=smtp with EMAIL_FROM + SMTP_HOST, else falls back to log', () => {
        expect(resolveEmailTransport(cfg({ EMAIL_PROVIDER: 'smtp', EMAIL_FROM: 'a@b.com', SMTP_HOST: 'smtp.mailgun.org' })).name).toBe('smtp');
