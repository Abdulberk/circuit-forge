...
describe('AllExceptionsFilter', () => {
        ...
        const quota = new HttpException(
            {
                ...
                limit: 5,
                period: '2026-06',
                message: 'Quota exceeded for design_jobs: 5 of 5 used this period.',
            },
            429,
        );
    ...
    it('PRESERVES a class-validator 400 (message array kept; code added; framework scaffolding stripped)', () => {
        const validator = new BadRequestException({
            statusCode: 400,
    ...
    it('an UNEXPECTED (non-HttpException) error → generic 500, NO leak, logged server-side with the stack', () => {
        ...
        const { statusArg, body } = run(
            new Error('connect ECONNREFUSED 10.0.0.5:5432 — prisma internals'),
            '/api/projects',
            'POST',
        );
