...
export class SaveWorkingCopyDto {
    @ApiProperty({ description: 'OUR CircuitJson — the current editable circuit', type: 'object', additionalProperties: true })
    @IsObject()
    circuitJson!: Record<string, unknown>;

    @ApiProperty({ description: 'Editor/layout UI state to persist alongside the circuit', type: 'object', additionalProperties: true })
    @IsObject()
    uiJson!: Record<string, unknown>;

    @ApiPropertyOptional({ description: 'The saved version this draft descends from (must belong to the same project)', format: 'uuid' })
    @IsOptional()
    @IsUUID()
