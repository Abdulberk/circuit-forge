/**
 * Orphan reaper — the recovery half of the durable PCB-layout queue (mirrors worker-sim's design reaper).
 *
 ...
 *   1. WORKER DIED MID-JOB — the row is RUNNING but the process that owned it is gone (rolling-deploy SIGKILL,
 *      OOM, node recycle, spot reclaim). Our QUEUED→RUNNING atomic claim makes a BullMQ redelivery a no-op
 *      (a second worker can't re-claim it), which also means the redelivery completes without writing a
 *      terminal row — so the row would sit RUNNING forever and the client's LRO poll never terminates.
 *   2. INSERT↔ENQUEUE GAP — create() inserts the QUEUED row, then enqueues; an API crash strictly between the
 *      two leaves a row QUEUED that no worker will ever see (it never became a queue job).
 *
 * This reaper reconciles such rows against the queue's GROUND TRUTH. Because the API sets the BullMQ job id =
 * the LayoutJob row id, we can look each row up (queue.getJob(rowId)) and act on its real state. It is
 ...
 * job), should the row be reaped, and why? Exported for testing — no I/O.
 *
...
export function reapDecision(row: ReapableRow, state: string, nowMs: number, runningDeadlineMs: number): ReapDecision {
    switch (state) {
        ...
        // worker is hung (a quality route + DRC is minutes; the deadline is far beyond that).
        case 'active': {
...
/**
 * One reaper sweep. Finds LayoutJob rows stuck in QUEUED/RUNNING past the grace window, asks the queue for
 ...
 */
export async function reapStaleLayoutJobs(deps: {
    prisma: {
        layoutJob: {
    ...
    const rows = await db.layoutJob.findMany({
        where: {
    ...
    for (const row of rows) {
        try {
            ...
            const res = await db.layoutJob.updateMany({
                where: { id: row.id, status: { in: ['QUEUED', 'RUNNING'] } },
                data: { status: 'FAILED', errorMessage: decision.reason, finishedAt: new Date(nowMs) },
            });
            if (res.count > 0) {
                reaped += res.count;
                log?.warn({ jobId: row.id, was: row.status, queueState: state, reason: decision.reason }, 'Reaped orphaned layout job');
            }
        } catch (e) {
    ...
    if (reaped > 0) log?.info({ examined: rows.length, reaped }, 'Layout-job reaper sweep complete');
    return { examined: rows.length, reaped };
}

/**
 * Start the periodic reaper (boot sweep + every REAPER_INTERVAL_MS). Owns its own Redis connection + Queue
 * handle (needed for getJob). Returns a handle whose stop() clears the timer and closes the connection — call
 * it on graceful shutdown. Each tick is isolated: a sweep error is logged, never crashes the loop.
 */
export function startLayoutReaper(): { stop: () => Promise<void> } {
    const connection = new Redis(config.REDIS_URL, { maxRetriesPerRequest: null });
    const queue = new Queue(config.PCB_QUEUE_NAME, { connection });

    const tick = async () => {
        try {
            await reapStaleLayoutJobs({
                ...
                prisma: prisma as unknown as Parameters<typeof reapStaleLayoutJobs>[0]['prisma'],
                queue,
                nowMs: Date.now(),
                graceMs: config.PCB_REAP_GRACE_MS,
                runningDeadlineMs: config.PCB_REAP_RUNNING_DEADLINE_MS,
                log: logger,
            });
        } catch (e) {
    ...
    timer.unref?.();
    logger.info({ intervalMs: config.REAPER_INTERVAL_MS, queue: config.PCB_QUEUE_NAME }, 'Layout-job reaper started');
