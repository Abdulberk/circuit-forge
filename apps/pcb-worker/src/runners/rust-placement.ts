...
function childError(error: unknown, binary: string, timeoutMs: number, maxBuffer: number): Error {
    ...
    const exit = typeof e.code === 'number' ? `exit ${e.code}` : e.code ? `code ${e.code}` : 'unknown exit';
...
function parseOutput(raw: string, input: PlacementInput): PlacementOutput {
    ...
    if (typeof value.ok !== 'boolean') throw new Error('Rust placement output field "ok" must be boolean.');
    ...
    if (!isRecord(value.positions)) throw new Error('Rust placement output field "positions" must be an object.');

    const expectedIds = new Set(input.parts.map((part) => part.id));
    const actualIds = Object.keys(value.positions);
    for (const id of actualIds) {
        if (!expectedIds.has(id)) throw new Error(`Rust placement output contains unknown part id "${id}".`);
        const position = value.positions[id];
        if (!isRecord(position) || !finite(position.x) || !finite(position.y)) {
            throw new Error(`Rust placement output position for "${id}" must contain finite x/y coordinates.`);
        }
        if (
            position.rotation !== 0 &&
    ...
    if (value.ok) {
        for (const id of expectedIds) {
            if (!(id in value.positions)) throw new Error(`Rust placement output is missing part id "${id}".`);
        }
    }
        ...
        try {
            ...
            try {
                ...
                if ((error as NodeJS.ErrnoException).code === 'ENOENT') {
                    throw new Error('Rust placement binary exited successfully but did not create output.json.');
                }
                throw error;
            }
