...
> Note: the worker now runs the **whole process as a non-root user** (`ngsim`) with **kernel resource limits** (§5.2) UNDER **container-level read-only-root, ALL capabilities dropped, no-new-privileges, and process/memory ceilings** (§5.3). Still NOT present by default: a **user-namespace / seccomp / network-egress** sandbox of the ngspice child (rootless bubblewrap). That layer is **host-conditional** — rootless userns is blocked under Docker's default seccomp unless the host kernel permits unprivileged user namespaces — so it must fail-loud + fall back to the controls above, and is left as an optional upgrade rather than a default.
