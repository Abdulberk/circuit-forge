...
- `getResult`: loads the job (`404` if missing), checks membership. If `status !== 'SUCCEEDED'` returns `{ id, status, error: stderr }`. If succeeded returns `{ id, status, result: resultJson, metrics }` (the result is read from the DB; an S3 fallback is noted as a TODO in code).
